...
void sensory_embedding_v(deque<double>& chassis_accel, deque<double>& chassis_angvel, deque<double>& left_wheelTorque, deque<double> &right_wheelTorque){
  y_v(0) = chassis_accel[1];
  y_v(1) = (chassis_angvel[2]-chassis_angvel[1])/dt;
  y_v(2) = left_wheelTorque[1];
  y_v(3) = right_wheelTorque[1];
  y_v(4) = (chassis_accel[1]-chassis_accel[0])/dt;
  y_v(5) = (chassis_angvel[2]-2*chassis_angvel[1]+chassis_angvel[0])/(dt*dt);
...
void AI_Setup(){
    ...
    A = kron(I_p,tmp_A);
    Eigen::MatrixXd tmp_B_v =  Eigen::MatrixXd::Zero(V_SIZE+2,V_SIZE);
    tmp_B_v<<1, 0,
             0, 1,
             0.5*M_C+1.5*M_W, -(1.5*M_W*d + I_C/(2*d)),
             0.5*M_C+1.5*M_W, -(1.5*M_W*d - I_C/(2*d));
    ...
    Eigen::MatrixXd tmp_Pi_z2(V_SIZE+2,V_SIZE+2);
    ...
    dF_dvv = A.transpose() * Pi_w * A + B_v.transpose()*Pi_z2*B_v;
...
void AI_Update(){
    ...
    FreeEnergy = E(0,0);
    Eigen::MatrixXd dF_dx= D_x.transpose()*(Pi_w*(eps_x))-B_x.transpose()*Pi_z1*(eps_y1);
